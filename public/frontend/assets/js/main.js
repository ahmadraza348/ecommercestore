...
(function ($) {
    ...
    // ----------- QTY BUTTON HANDLER WITH STOCK LIMIT -------------
    proQty.on("click", ".qtybtn", function () {
        let btn = $(this);
        let input = btn.parent().find("input");
        let oldVal = parseInt(input.val());
        let maxStock = parseInt(input.attr("max")) || 99999;

        let newVal = oldVal;

        if (btn.hasClass("inc")) {
            newVal = Math.min(oldVal + 1, maxStock);
        } else {
            newVal = Math.max(oldVal - 1, 1);
        }
    ...
    if (map_id.length > 0) {
        ...
        L.tileLayer(
            "https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}",
            {
                ...
                accessToken:
                    "pk.eyJ1Ijoic2hha2lsYWhtbWVlZCIsImEiOiJjamk4anF6NDgwMGd5M3BwM2c4eHU5dmIzIn0.yBLGUAB8kV1I1yGGonxzzg",
            }
        ).addTo(mymap);

        var marker = L.marker([$lat, $lng]).addTo(mymap);
        marker
            .bindPopup("<b>" + $maptitle + "</b><br>" + $mapaddress)
            .openPopup();
