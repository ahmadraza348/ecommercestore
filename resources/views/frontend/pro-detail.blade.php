...
@section('content')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            let colorRadio = storedColorId ? $(`input[name="color"][value="${storedColorId}"]`) : $(
        ...
        let selected = $('input[name="color"]:checked');
        $('#selected-color-name').text(selected.data('name'));
        ...
        function loadVariantValues(variants) {
            ...
            let html =
                `<label><strong>Select ${attributeName}: ${attributeValue}</strong></label><div class="variant-options">`;
        ...
        <label for="variant_${v.id}">${v.attribute_value.name}</label>`;
            ...
            $('input[name="variant"]').off('change').on('change', function() {
                ...
                const vName = $(this).next('label').text(); // get the selected variant name
